...
# Template the file
((Get-Content -path $outfile -Raw) -replace "{{MACHINE_USER_PASSWORD}}","$Env:MACHINE_USER_PASSWORD" -replace "{{MACHINE_USERNAME}}","$Env:MACHINE_USERNAME" -replace "{{MACHINE_NAME}}","$Env:MACHINE_NAME") | Set-Content -Path $outfile